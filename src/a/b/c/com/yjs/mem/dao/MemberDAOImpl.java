...
import a.b.c.com.yjs.mem.sql.MemberSqlMap;
...
public class MemberDAOImpl implements MemberDAO {
		...
		try {
			...
			if (rsRs !=null) {
				...
				while (rsRs.next()) {
					...
					// 변수를 언더바로 시작하는 게 위험해서 요즘은 뒤에다 붙이는 추세~ 
			...
			System.out.println("memberSelectAll()에서 디비연결 또는 쿼리에서 문제가 생겼네요 >>> : " + e.getMessage());
		}finally {
			ConnProperty.conClose(conn, pstmt, rsRs);
		}
	...
	public ArrayList<MemberVO> memberSelect(MemberVO mvo) {
		...
		try {
			...
			System.out.println("memberSelect()에서 디비연결 또는 쿼리에서 문제가 생겼네요 >>> : " + e.getMessage());
		}finally {
			ConnProperty.conClose(conn, pstmt, rsRs);
		}
	...
	public boolean memberUpdate(MemberVO mvo) {
		...
		try {			
			...
			System.out.println("수정 디비 연동에 문제가 생겼습니다. >>> : " + e);
		}finally {
			try {
				ConnProperty.conClose(conn, pstmt);				
			}catch(Exception e2) {}
	...
	public boolean memberDelete(MemberVO mvo) {
		...
		try {			
			...
			System.out.println("삭제 디비 연동에 문제가 생겼습니다. >>> : " + e);
		}finally {
			try {
				ConnProperty.conClose(conn, pstmt);				
			}catch(Exception e2) {}
