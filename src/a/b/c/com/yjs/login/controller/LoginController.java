package a.b.c.com.yjs.login.controller;
...
import a.b.c.com.yjs.login.service.LoginService;
import a.b.c.com.yjs.login.service.LoginServiceImpl;
		...
		if (login_type !=null && login_type.length() > 0) {
			...
			if("LOGIN".equals(login_type)){
				...
				if (nCnt == 1) {
					...
					RequestDispatcher rd= request.getRequestDispatcher("/yjs/login/login.jsp");
					rd.forward(request, response);					
				}else {
					out.println("<script>");	
					out.println("alert('로그인 실패 !!')");	
					out.println("location.href='/testYjs/yjs/login/login.html'");
					out.println("</script>");
				}					
			}	
			...
			if("LOOUT".equals(login_type)){
				...
				if (bool) {
					RequestDispatcher rd= request.getRequestDispatcher("/yjs/login/logout.jsp");
					rd.forward(request, response);					
				}
				else {}					
			}
