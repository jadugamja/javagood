package a.b.c.com.yjs.book.controller;
...
import a.b.c.com.yjs.book.service.BookService;
import a.b.c.com.yjs.book.service.BookServiceImpl;
		...
		if (isudtype !=null && isudtype.length() > 0){
			...
			if ("SALL".equals(isudtype)){
				...
				if (aListAll !=null && aListAll.size() > 0){
					...
					RequestDispatcher rd= request.getRequestDispatcher("/yjs/book/bookSelectAll.jsp");
					rd.forward(request, response);
					
				}
				else{
					out.println("<script>");					
					out.println("location.href='/testYjs/book?isudtype=SALL'");
					out.println("</script>");
				}		
			}
			...
			if ("S".equals(isudtype) || "U".equals(isudtype) || "D".equals(isudtype)){
				...
				if (bnum !=null && bnum.length() > 0){
					...
					if (aList !=null && aList.size() > 0) {
						...
						RequestDispatcher rd= request.getRequestDispatcher("/yjs/book/bookSelect.jsp");
						rd.forward(request, response);
						
					}
					else{
						out.println("<script>");					
						out.println("location.href='/testYjs/book?isudtype=S'");
						out.println("</script>");
					}									
				}
				else{
			...
			if ("UOK".equals(isudtype)){
				...
				if (nCnt >  0) {
					...
					RequestDispatcher rd= request.getRequestDispatcher("/yjs/book/bookUpdate.jsp");
					rd.forward(request, response);
					...
					out.println("<script>");					
					out.println("location.href='/testYjs/book?isudtype=U'");
					out.println("</script>");
				}
			}
			...
			if ("DOK".equals(isudtype)){
				...
				if (nCnt >  0) {
					...
					request.setAttribute("nCnt", new Integer(nCnt));										
					RequestDispatcher rd= request.getRequestDispatcher("/yjs/book/bookDelete.jsp");
					rd.forward(request, response);
					...
					out.println("<script>");					
					out.println("location.href='/testYjs/book?isudtype=D'");
					out.println("</script>");
				}
			}			
			...
			if ("I".equals(isudtype)){
				...
				if (nCnt >  0) {
					...
					request.setAttribute("nCnt", new Integer(nCnt));		
					//getRequestDispatcher()는 Context 생략ㄱㄱㄱㄱㄱ
					RequestDispatcher rd= request.getRequestDispatcher("/yjs/book/bookInsert.jsp");
					rd.forward(request, response);
					...
					System.out.println("도서 정보 등록 실패 !!!!");
					out.println("<script>");
					// ?????? location.href='/testYjs/book?isudtype=D'
					out.println("location.href='/testYjs/book/book.html'");
					out.println("</script>");
				}
			}
		}
